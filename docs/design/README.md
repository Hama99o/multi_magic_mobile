...
| 10 | **Notifications** | `api/v1/notifications` (**six endpoints, not three** — `unread_count`, `read_all`, `clear`) + `notification_channel` | **`IN PROGRESS`** → [`notifications/`](notifications/SPEC.md) | **a tap composes a question, it does not open a record** — so **no chevron on a row**, which is the rejection that carries the architecture |
| 11 | **Calendar — the important view** | `api/v1/calendar_app/events/**upcoming**` — NOT `/events`, which dates a recurring birthday to 1990 | **`IN PROGRESS`** → [`calendar/`](calendar/SPEC.md) | agenda, never a month grid; today says so when empty; still the first thing I would cut if time gets tight |
| 12 | **People chat** — threads with humans | `resources :conversations` + `messages` + `reactions` + `mark_read` · messages over **`MessageChannel`**, typing/read over `ConversationChannel` | **`IN PROGRESS`** → [`people-chat/`](people-chat/SPEC.md) | **the same model as the assistant, separated by one boolean** (`is_ai`) — and the two screens must be **unmistakable** for each other, his instruction: §2.4 |
| 13 | **Privacy policy** — rendered in the app | **none exists** — no route, no page; the text is `docs/PRIVACY.draft.md`, rendered from `src/content/privacy.generated.ts` | **`IN PROGRESS`** → [`account/`](account/SPEC.md) | **a store requirement**, both stores. Renders in-app, never a web view; the internal notes are stripped at build and a test fails if they ever ship. **Still says DRAFT until he approves it — that banner is a release gate** |
| 14 | **Delete account — everything** | **`BLOCKED`: the endpoint does not exist.** `users_controller.rb:134` is a commented-out attempt using **`delete`, not `destroy`** — which would orphan all 57 dependents | **`IN PROGRESS`** → [`account/`](account/SPEC.md) | built against a stub and honestly gated: the confirm names what **goes** and what is **kept** (`ai_usage_events` is `:nullify`), and nobody is walked through a password into a wall. **It must not look like the conversation delete two rows away**, which is safe by construction |
| 15 | Error & empty states | — | `SPECIFIED` in each | 429, `aiError` over the socket, no recogniser, permission refused, offline |
| 16 | **Profile** — photo, details, password, your own AI key | `users/connected_user` · `PATCH users/:id` (photo is `user[photo]`) · `PUT :change_password` · `destroy_avatar` · `ai_keys` | **`IN PROGRESS`** → [`profile/`](profile/SPEC.md) | his instruction, 18 Sept. **The email is shown and LOCKED** — the socket identifies the user by it, so changing it kills the cable silently. A wrong current password is a **422 and must not sign you out**; the server chose that status for this client |
